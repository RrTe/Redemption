...
class PlayerFactory {
  ...
  static createPlayer(client, cardLookup) {
    ...
    const { mainDefs, reserveDefs } = DeckService.resolveDeck(
      client.userData.deck,
    );
    ...
    logger.info(
      `[PlayerFactory] PlayerState created for ${p.name} (${p.sessionId})`,
    );
    // Nur ein kurzes Log hier, um Flooding zu vermeiden, aber Bestätigung zu geben
    return p;
  }
