...
export class DeckEditorScene extends Phaser.Scene {
  ...
  private showDeckMetrics() {
    ...
    // Retrieve or create the DOM overlay from the preloaded HTML template cache.
    let domElement = this.children.getByName(
      "deckMetricsDOM",
    ) as Phaser.GameObjects.DOMElement;
    if (!domElement) {
      // Create DOM element from Phaser's preloaded HTML cache
      domElement = this.add
        .dom(viewW / 2, viewH / 2)
    ...
    if (!overlay) {
      console.error(
        "[DeckEditorScene] Could not create or find deckMetrics DOM overlay.",
      );
      return;
    }
    ...
    // Pause the Phaser scene while the metrics overlay is open so no
    // input events pass through to the card browser behind it.
    this.scene.pause();
    ...
    requestAnimationFrame(() => {
      ...
      // 1. Definiere die Wunschgröße des Overlays (JS steuert als Single Source of Truth die Maße und Skalierung)
      const baseW = 660; // Breite auf 660px erhöht
      const baseH = 970; // Höhe auf 930px erhöht
      log(
        "DeckEditorScene",
        "DeckMetrics initialized with size:",
        baseW,
        "x",
        baseH,
      );
      ...
      // Positioniere das innere Element absolut relativ zum zentrierten Phaser-Wrapper
      overlay.style.position = "absolute";
      overlay.style.top = "0px";
      overlay.style.left = "0px";
      overlay.style.margin = "0px";
      overlay.style.transformOrigin = "center center"; // Wichtig: Skalierung aus der Mitte!

      // 2. Berechne den Skalierungsfaktor basierend auf Breite UND Höhe (Proportional)
      const scaleX = currentW / baseW;
      const scaleY = (currentH - 20) / baseH;
      const scaleFactor = Math.min(1.0, scaleX, scaleY); // Vergrößert nicht über 100% der Wunschgröße

      // 3. Wende die proportionale Skalierung und Zentrierung via CSS-Transform auf das innere Element an
      overlay.style.transform = `translate(-50%, -50%) scale(${scaleFactor})`;

      // 4. Setze das Phaser-Wrapper-Div stabil auf die Bildschirmmitte (Origin 0,0)
      domElement.setPosition(currentW / 2, currentH / 2);
      domElement.setOrigin(0, 0);
      domElement.setScale(1);
    });

    // Wire up the close button to also resume the Phaser scene.
    // We replace the old onclick with a new handler each time to avoid stacking.
    const closeBtn = document.getElementById("closeCardMetricsDiv");
    if (closeBtn) {
      // Clone-replace to strip all previous listeners, then add fresh one
      ...
      freshBtn.addEventListener(
        ...
        (e) => {
          ...
          // Resume the Phaser scene after closing the overlay
          this.scene.resume();
        },
        { once: true },
      );
    ...
    // Helper: update a bar element height in pixels (100% corresponds to 100px baseline, scaled to 1.2 for height)
    const updateBarChart = (
      ...
      if (barEl) {
        barEl.style.height = pct * 2.5 + "px";
      }
      const txtEl = document.getElementById(elementID + "_txt");
      if (txtEl) txtEl.textContent = String(count);
      const pctEl = document.getElementById(elementID + "_percentage");
      if (pctEl) pctEl.textContent = pct.toFixed(0) + "%";
    };

    // Helper: update a ratio label
    const updateRatio = (elementID: string, value: string) => {
      const el = document.getElementById(elementID);
      if (el) el.textContent = value;
    };

    // Helper: update brigade distribution cells (characters + enhancements per color)
    const updateBrigade = (color: string, alignment: "Good" | "Evil") => {
      ...
      if (alignment === "Good") {
        ...
        charType = "Evil Character";
        charAlt = "DAC";
        enhType = "EE";
        enhAlt = "Curse";
      }

      const chars = this.deckListModel
        .cardsByBrigadeType(charType, color)
        .concat(this.deckListModel.cardsByBrigadeType(charAlt, color));
      const enhs = this.deckListModel
        .cardsByBrigadeType(enhType, color)
        .concat(this.deckListModel.cardsByBrigadeType(enhAlt, color));

      const charTC = chars.filter((c) => {
        const cls = Array.isArray(c.Class) ? c.Class : [c.Class];
        return cls.some((v: string) => v && v.indexOf("Territory") !== -1);
      }).length;
      ...
      const charTCEl = document.getElementById(
        `${colorLower}_${charType}_TC_no`,
      );
    ...
    const total = this.deckListModel.deck.reduce((s, e) => s + e.quantity, 0);
    const numHeroes =
      this.deckListModel.cardsByType("Hero").length +
      this.deckListModel.cardsByType("DAC").length;
    const numGEs =
      this.deckListModel.cardsByType("GE").length +
      this.deckListModel.cardsByType("DAE").length +
      this.deckListModel.cardsByType("Covenant").length;
    const numECs =
      this.deckListModel.cardsByType("Evil Character").length +
      this.deckListModel.cardsByType("DAC").length;
    const numEEs =
      this.deckListModel.cardsByType("EE").length +
      this.deckListModel.cardsByType("DAE").length +
      this.deckListModel.cardsByType("Curse").length;
    const numDoms = this.deckListModel.cardsByType("Dominant").length;
    const numArts =
      this.deckListModel.cardsByType("Artifact").length +
      this.deckListModel.cardsByType("Covenant").length +
      this.deckListModel.cardsByType("Curse").length;
    const numForts =
      this.deckListModel.cardsByType("Fortress").length +
      this.deckListModel.cardsByType("City").length;
    const numSites =
      this.deckListModel.cardsByType("Site").length +
      this.deckListModel.cardsByType("City").length;
    const numSouls =
      this.deckListModel.cardsByType("Lost Soul").length +
      this.deckListModel.cardsByType("City").length;
    ...
    updateRatio(
      "characterRatio",
      numECs > 0 ? (numHeroes / numECs).toFixed(1) : String(numHeroes),
    );
    updateRatio(
      "heroGERatio",
      numGEs > 0 ? (numHeroes / numGEs).toFixed(1) : String(numHeroes),
    );
    updateRatio(
      "ecEERatio",
      numEEs > 0 ? (numECs / numEEs).toFixed(1) : String(numECs),
    );
    ...
    updateBrigade("Blue", "Good");
    updateBrigade("Clay", "Good");
    updateBrigade("Gold", "Good");
    updateBrigade("Green", "Good");
    updateBrigade("Purple", "Good");
    updateBrigade("Red", "Good");
    updateBrigade("Silver", "Good");
    updateBrigade("Teal", "Good");
    updateBrigade("White", "Good");
    updateBrigade("Multi", "Good");
    updateBrigade("Black", "Evil");
    updateBrigade("Brown", "Evil");
    updateBrigade("Crimson", "Evil");
    updateBrigade("Gold", "Evil");
    updateBrigade("Gray", "Evil");
    updateBrigade("Orange", "Evil");
    updateBrigade("Pale Green", "Evil");
    updateBrigade("Multi", "Evil");
  }
