...
export class CardDetailDomBuilder {
  ...
  public setImageSource(targetSrc: string): void {
    ...
    if (this.imgNode.src === targetSrc && this.imgNode.style.visibility === "visible") {
      return;
    }
    this.imgNode.style.visibility = "hidden";
    this.imgNode.onload = () => {
      if (this.imgNode && this.imgNode.src === targetSrc) {
        this.imgNode.style.visibility = "visible";
      }
    };
    this.imgNode.onerror = () => {
      if (this.imgNode) this.imgNode.style.visibility = "visible";
    };
    this.imgNode.src = targetSrc;
    if (this.imgNode.complete && this.imgNode.naturalWidth > 0) {
      this.imgNode.style.visibility = "visible";
    }
  }
  ...
  public applyFloatingStyles(
    ...
    Object.assign(this.panelNode.style, {
      height: "auto",
      maxHeight: `${Math.round(targetHeight)}px`,
