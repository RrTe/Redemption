...
async def test_ruling_flow():
    ...
    test_question = (
        ...
        "And if so, does the special inish off it or the enhancement come first?"
    )

    print(f"\n💬 [Schritt 2] Sende 'ruling question' an die Engine...")
    print(f'   Frage: "{test_question}"')
    print(
        "   (Die KI analysiert nun Kontext, Regeln und historische Discord-Rulings...)\n"
    )
    ...
    try:
        ...
        print(
            f"📊 DISCORD EMBED SIMULATION ({paginated.total_pages} Seite/n generiert):"
        )
