...
LOG_UNKNOWN_SETS = True
LOG_VALIDATION_KEYS = True
LOG_SET_VARIANTS = True
LOG_NOT_FOUND = True
LOG_ORDIR_STRUCTURE = True
...
def extract_set_variants(raw_set):
    if LOG_SET_VARIANTS:
        print(f"🧪 extract_set_variants: Eingabe = {repr(raw_set)}")
    variants = [raw_set.strip()] if raw_set else []
    if LOG_SET_VARIANTS:
        print(f"🧪 extract_set_variants: Ausgabe = {variants}")
    return variants
...
def is_marker(line):
    ...
    return "the following redemption" in line and (
        ...
        "represent" in line or
        "have" in line and "in the title" in line
    )
...
def parse_card_line(line):
    ...
    if ":" not in line:
        if LOG_PARSE_CARD_LINE:
            print(f"⚠️ Ungültige Bullet-Zeile (kein ':'): {repr(line)}")
        return results

    category_part, cards_part = line.split(":", 1)
    category_part = category_part.strip()
    cards_part = cards_part.strip()
    card_parts = re.findall(r"[^,()]+?\([^)]+\)", cards_part)
    ...
    for part in card_parts:
        ...
        raw_sets = [s.strip() for s in set_part.split(",")]
        resolved_sets = []
        if LOG_PARSE_CARD_LINE:
            print(f"  🔍 Set-String: {set_part} → raw_sets = {raw_sets}")

        for raw in raw_sets:
            if raw in SET_ALIAS:
                resolved_sets.append(SET_ALIAS[raw])
            elif "/" in raw and raw not in SET_ALIAS:
                for sub in raw.split("/"):
                    sub = sub.strip()
                    if sub in SET_ALIAS:
                        resolved_sets.append(SET_ALIAS[sub])
            else:
                if LOG_UNKNOWN_SETS:
                    print(f"⚠️ Unbekanntes Set: {raw}")
                    
        if LOG_PARSE_CARD_LINE:
            print(f"  ✅ resolved_sets = {resolved_sets}")

        for resolved in resolved_sets:
            if isinstance(resolved, list):
                results.extend([(name_part, r) for r in resolved])
            else:
                results.append((name_part, resolved))

    if LOG_PARSE_CARD_LINE:
        print(f"🔍 parse_card_line: Ergebnisse aus Zeile → {repr(line)}")
        for r in results:
            print(f"  → {r}")
    return results
        ...
        while i < len(lines):
            ...
            if is_marker(marker):
                if LOG_ORDIR_STRUCTURE:
                    ...
                    print(f"   → Marker ist Referenz: {is_reference_marker(marker)}")
