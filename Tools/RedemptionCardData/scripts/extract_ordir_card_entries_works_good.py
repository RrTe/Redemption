...
def strip_print_suffix(name):
    name = str(name)
    name = re.sub(r"\s*\((1st|2nd|Alt|Alternate|Promo|Foil|Print|Art|Version)[^)]+\)", "", name)
    name = re.sub(r"\s*\((L1P|L2P|P|Promo|Alt-Art|Foil|Print)\)", "", name)
    name = re.sub(r"\s*\(.*?Print.*?\)", "", name)
    return name.strip()
...
try:
    ...
    for idx, card in enumerate(extended_cards):
        ...
        stripped_name = strip_print_suffix(name)
        if name != stripped_name:
            print(f"   → Printversion erkannt: {repr(name)} → {repr(stripped_name)}")
        name = stripped_name
